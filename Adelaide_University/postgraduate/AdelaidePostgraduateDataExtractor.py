                    ...
                    for lable in grid_items.find_all(class_='c-degree-finder__detail-grid-label'):
                        if 'Degree' in lable.text:
                            ...
                            if level_code == 'Bachelor':
                                ...
                                code = 'FOUND'
                                level_codes.append(code)
                            elif level_code == 'Masters by Coursework':
                                code = 'MST'
                                level_codes.append(code)
                            elif level_code == 'Professional Certificate':
                                code = 'VOC'
                                level_codes.append(code)
                            elif level_code == 'Graduate Diploma':
                                code = 'GDIP'
                                level_codes.append(code)
                            elif level_code == 'Graduate Certificate':
                                code = 'GCERT'
                                level_codes.append(code)
                            else:
                ...
                for grid_items in narrow_down:
                    for lable in grid_items.find_all(class_='c-degree-finder__detail-grid-label'):
                        if 'Duration' in lable.text:
                            durr = lable.find_next('span').text
                            if 'year' in durr:
                                ...
                                dur = lable.find_next('span').text.split('month')
                                duration = dur[0].strip()
                                print(duration)
                                durations.append(duration)
            else:
                                ...
                                for td in tr_tag.find_all('td'):
                                    if "$" in td.text:
                                        ...
                                        if 'for' not in (fee):
                                            ...
                                            fee_ = fee.split('f')
                                            feee = fee_[0].strip()
                                            total_fee = float(feee.replace(',', ''))*float(duration)
                                            print('{:,}'.format(total_fee))
                                            int_fees.append('{:,}'.format(total_fee))
...
test_df.to_csv(r'/Users/zeinalabidin/desktop/CSV_files/AdelaideUniversityPostgraduateCourses251-270.csv', index=False,header=True)
